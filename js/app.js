...
const cStandForm = document.getElementById('cstand-form');
// const cStandFormAmt = document.getElementById('');
// const cStandFormTime = document.getElementById('');

function addCStandHandler(event) {
  event.preventDefault();
  const cityName = event.target.city.value;
  const whatTime = event.target.time.value;
  const howMany = event.target.amount.value;
...
cStandForm.addEventListener('submit', addCStandHandler);

//to do: footer. do i need const funcs?
const CookieStand = function(cityName,howMany,whatTime) {
  ...
  function renderCookieStand(CookieStand) {
    ...
    for (let i = 0; i < 1; i += 1) {
      ...
      const currentTime = whatTime;

      // const currentSales = cookieStand.hourlySales[i];

      headCellElem.textContent = currentTime;
    }
    ...
    for (let i = 0; i < 1; i += 1) {
      ...
      for (let cookieInd = 0; cookieInd < cityName.howMany[cookieInd]; cookieInd += 1) {
        const cellElem = document.createElement('td');
        // console.log(locations[i][1]);
        rowElem.appendChild(cellElem);

        const currentCookie = cityName.howMany[cookieInd];
    ...
    renderCookieStand(cityName,howMany,whatTime);
  }
};
